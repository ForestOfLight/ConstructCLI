...
on:
  ...
  workflow_dispatch:
    inputs:
      dry_run:
        description: "Build and upload artifacts without creating a release"
        type: boolean
        default: true
...
jobs:
  ...
  verify:
    ...
    steps:
      ...
      - name: Check release preconditions
        if: github.event_name == 'push'
        run: ./scripts/check-release-preconditions.sh "${{ github.ref_name }}"
  ...
  build:
    ...
    strategy:
      ...
      matrix:
        include:
          ...
          # fail on those systems with an unhelpful symbol error.
          - os: ubuntu-22.04
    ...
    steps:
      ...
      # Generated per-runner so each archive's notices describe the binary it
      # actually ships with, including platform-specific dependencies.
...
From the GitHub Actions tab, run the `Release` workflow manually against the `release-pipeline` branch with `dry_run` left checked. Nothing is published — `publish` is skipped on `workflow_dispatch`.
